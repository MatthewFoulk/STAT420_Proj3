...
ci_95_norm <- function(n){
 ...
 CI_95_exp <- replicate(B,
                            {samp <- rexp(n, rate = 4)
                            mean_inside_CI(samp, pop_mu = 1/4 , pop_sig = 1/16)  # I CANNOT GET THIS TO WORK, MY MACHINE REFUSES TO FIND mean_inside_ci :(((, do we have to make it or is it in the library that won't work on my machine (dplyr)?
                            })
...
n <- 50
B <- 100000
CI_95_exp2 <- replicate(B, {samp <- rexp(n,rate=4)})
...
Y <- function(alpha, beta, sig){
  alpha + beta*x_fixed + rnorm(1,0,sig)
}
...
Y_hat <- function(samp, x){
  ...
  return(alpha_hat(samp) + beta_hat(samp)*x+epsilon)
}
```
...
```{r}
plot(x_fixed, true_line, type = 'l')
for (i in seq(1,50)){
  lines(x_fixed,Y(3,5,1.6))
}
```
